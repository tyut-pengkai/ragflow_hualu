...
export function Header() {
  ...
  const tagsData = useMemo(
    () => {
      const allTags = [
        { path: Routes.Datasets, name: t('knowledgeBase'), icon: Library, requireSuperuser: true },
        { path: Routes.Chats, name: t('chat'), icon: MessageSquareText, requireSuperuser: false },
        { path: Routes.Search, name: t('search'), icon: Search, requireSuperuser: false },
        { path: Routes.Agent, name: t('flow'), icon: Cpu, requireSuperuser: true },
        { path: Routes.Files, name: t('fileManager'), icon: File, requireSuperuser: true },
      ];

       // 使用严格的布尔值比较
       if (userInfo?.is_superuser !== true) {
        return allTags.filter(tag => !tag.requireSuperuser);
      }
    ...
    return tagsData.map((tag) => ({
      label: (
        <div className="flex items-center gap-1">
          <tag.icon className="size-5"></tag.icon>
          <span>{tag.name}</span>
        </div>
      ),
      value: tag.path,
    ...
    <section className="py-6 px-10 flex justify-between items-center border-b">
      ...
      <div className="flex items-center gap-4">
        ...
        <Container className="px-3 py-2 bg-colors-background-inverse-standard">
          ...
          <span className="max-w-14 truncate"> mxy2479@hlet.com</span>
          <Button
            variant="destructive"
